...
def main():
    ...
    #data_yaml_path ='.\DATASET_YOLO_v3_baseline\data.yaml' # 상대 경로 사용
    data_yaml_path = './DATASET_YOLO_v4_perfect/data.yaml'
    ...
    results = model.train(
        ...
        imgsz=800,
        batch=16,
        project='./runs_local',      # <-- 1. 최상위 폴더 설정
        name='pill_detection_yolov8s_v4_800', # 학습 결과가 저장될 폴더 이름
        device='cuda'  # <-- ★★★  GPU 사용 ★★★
    )
