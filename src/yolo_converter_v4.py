...
AUG_IMAGE_DIR = os.path.join(PREPROCESSED_DIR, 'augmented_images_v4_perfect') # (!!!) 이름 확인!

# [입력 3] V2 증강된 훈련 마스터 CSV (clean 버전)
TRAIN_CSV_PATH = os.path.join(PREPROCESSED_DIR, 'train_aug_master_v4_perfect.csv') # (!!!) 이름 확인!
# [입력 4] V1 원본에서 분리된 깨끗한 검증 CSV (!!!)
VAL_CSV_PATH = os.path.join(PREPROCESSED_DIR, 'val_set_v4_clean.csv') # (!!!) 이름 확인!

# [출력] V2 최종 YOLO 데이터셋 루트 폴더
YOLO_DATASET_ROOT = os.path.join(BASE_PROJECT_PATH, 'DATASET_YOLO_v4_perfect') # (!!!) 새 폴더 이름!
...
if __name__ == "__main__":
    ...
    process_split(df_train, CLASS_TO_ID_MAP, ORIGINAL_IMAGE_DIR, AUG_IMAGE_DIR, train_img_dst, train_label_dst)
